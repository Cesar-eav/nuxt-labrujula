<template>
  <div>
    ...
    <div  class="grid grid-cols-1 md:grid-cols-2 md:mx-10 px-4 md:gap-5 gap-y-5 justify-center my-2">
      ...
      <img :src="baseurl+'/storage/' + atractivo.image" class="max-h-32 w-32 pr-2 md:pr-2 md:max-h-60 md:w-60 object-contain rounded-full"   />
...
export default {
  ...
  methods: {
    listCentros() {
      axios
        this.$axios.get("/api-centros-culturales") // Va a web.php por defecto y busca el nombre de la ruta que arroja el JSON
